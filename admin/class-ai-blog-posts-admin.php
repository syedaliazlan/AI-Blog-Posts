...
class Ai_Blog_Posts_Admin {
	...
	/**
	 * AJAX handler: Verify Pexels API key.
	 *
	 * @since    1.0.0
	 */
	public function ajax_verify_pexels_api_key() {
		check_ajax_referer( 'ai_blog_posts_nonce', 'nonce' );
		...
		$api_key = isset( $_POST['pexels_api_key'] ) ? sanitize_text_field( wp_unslash( $_POST['pexels_api_key'] ) ) : '';

		if ( empty( $api_key ) ) {
			wp_send_json_error( array( 'message' => __( 'Pexels API key is required.', 'ai-blog-posts' ) ) );
		}

		// Temporarily set the API key for verification
		$pexels = new Ai_Blog_Posts_Pexels();
		$result = $pexels->verify_api_key( $api_key );

		if ( $result['success'] ) {
			// Save the API key
			Ai_Blog_Posts_Settings::set( 'pexels_api_key', $api_key );
			wp_send_json_success( $result );
		} else {
			wp_send_json_error( $result );
		}
	...
	public function ajax_save_settings() {
		...
		// Note: Scheduling has been removed in simplified version
		// Settings are saved but no cron scheduling occurs
	...
	/**
	 * AJAX handler: Start step-by-step generation.
	 *
	 * @since    1.0.0
	 */
	public function ajax_start_generation() {
		...
		// Update queue status if from queue
		if ( $queue_topic_id ) {
			global $wpdb;
		...
		$job_id = $generator->start_job( $topic, array(
			'keywords'       => $keywords,
		...
		wp_send_json_success( array(
			'job_id'    => $job_id,
			'next_step' => 'outline',
			'message'   => __( 'Starting generation...', 'ai-blog-posts' ),
		) );
	}
	...
	public function ajax_process_step() {
		...
		if ( empty( $job_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Job ID is required.', 'ai-blog-posts' ) ) );
		}

		@set_time_limit( 120 );

		$generator = new Ai_Blog_Posts_Generator();
		$result = $generator->process_step( $job_id );

		if ( is_wp_error( $result ) ) {
			// Update queue status if failed
			if ( $queue_topic_id ) {
				global $wpdb;
				$table = $wpdb->prefix . 'ai_blog_posts_topics';
				$wpdb->update(
		...
		if ( $result['status'] === 'completed' && $queue_topic_id ) {
			global $wpdb;
			$table = $wpdb->prefix . 'ai_blog_posts_topics';
			$wpdb->update(
				$table,
	...
	public function ajax_generate_from_queue() {
		...
		$result = $generator->generate_post( $topic->topic, array(
			...
			// publish and generate_image will use defaults from settings
		) );
	...
	/**
	 * AJAX handler: Fetch trending topics (disabled in simplified version).
	 *
	 * @since    1.0.0
	 */
	public function ajax_fetch_trending() {
		...
		// Trending topics feature removed in simplified version
		wp_send_json_error( array( 
			'message' => __( 'Trending topics feature is not available in this version.', 'ai-blog-posts' ) 
		) );
	}
